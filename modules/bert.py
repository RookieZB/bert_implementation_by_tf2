...
class BERT(keras.layers.Layer):
    def __init__(self, config, model='bert', mode='seq', fromtf=True, **kwargs):
        super(BERT, self).__init__(**kwargs)
        self.model, self.mode, self.cat, self.fromtf = model.lower(), mode.lower(), 0, fromtf
        ...
        self.torchrpl = {'/': '.', 'layer_': 'layer.', 'kernel': 'weight', '_embeddings': '_embeddings.weight'}
        self.rpl = {'': ''} if self.fromtf else self.torchrpl
    ...
    def loading(self, ckpt):
        ...
        r1 = re.compile('|'.join(map(re.escape, self.rpl)))
        n1 = [r1.sub((lambda x1: self.rpl[x1.group(0)]), i1.name[:-2]) for i1 in self.weights]
        l1 = torch.load(ckpt) if not self.fromtf else None
